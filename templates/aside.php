
<aside>
    <section>
        <nav>
            <ul>
                <li><a class="lien_aside"  href="/home"> <img class="image_nav" src="../static/image/accueil.png" alt="Home"><p>Home</p></a></li>
                <li><a class="lien_aside"  href="/artiste_suivi"> <img class="image_loupe" src="../static/image/loupe.png" alt="Search"><p>Mes artistes suivis</p></a></li>
                <li><a class="lien_aside"  href="/playlists"> <img class="image_librairie" src="../static/image/librairie.png" alt="Library"><p>Mes playlists</p></a></li>
                <li><a class="lien_aside"  href="/create_playlist"> <img class="image_add" src="../static/image/add.png" alt="create"><p>Créer une playlist</p></a></li>
            </ul>
        </nav>
    </section>
    <section class="sectionAside">
        <a  onclick="window.location.href='/login'"><img class="boutton_aside" src="./static/image/profile.png" alt=""></a>
        <a  onclick="window.location.href='/login'"><img class="boutton_aside" src="./static/image/deconexion.png" alt=""></a>
    </section>
</aside>